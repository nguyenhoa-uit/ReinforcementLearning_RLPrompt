...
class PromptedClassificationEvaluator:
    ...
    def forward(
        ...
        # 'no movement , no yuks , not much of anything . RatingFilmRatingGrade Grade <mask> .'

            current_prompts = [self.prompt for _ in range(batch_size)]
            formatted_templates = self._format_prompts(current_prompts, inputs)

# tensor([[40.4769, -4.6984, 51.7547,  ...,  0.6816,  1.3315, 28.7418],
#         [36.1492, -4.2001, 49.1443,  ...,  0.5923,  1.2580, 26.0841],
#         [36.1594, -4.0799, 50.1343,  ...,  0.6000,  1.1382, 26.0409],
#         ...,
                      
#         [39.8482, -4.3326, 52.6479,  ...,  1.1549,  1.6525, 28.4031],
#         [39.7569, -4.3325, 52.9622,  ...,  1.0909,  1.6679, 28.0885],
#         [36.5115, -4.0646, 50.8199,  ...,  0.9824,  1.4023, 26.3248]],
#        device='cuda:0')

            all_logits = self._get_logits(formatted_templates)

        #     tensor([[0.7119, 0.2881],
        # [0.9623, 0.0377],
        # [0.9059, 0.0941],
            
            print(all_logits[:5, self.verbalizer_ids])
